...
```mermaid
graph TD
    A["Unit tests<br/>prescription, progression, volume, migration"] --> C["Handler tests<br/>real RPCs + real DB"]
    C --> D["API invariant harness<br/>randomised sequences"]
    D --> E["Cross-language golden fixtures<br/>Rust ↔ Dart parity"]
```

```bash
cargo test --all-targets   # backend: units, migration, handler tests
...
## Unit tests — the engine

Pure and fast, inline in their modules:

- `src/exercise_catalog.rs` — every exercise is classified (the scan is
  pinned, so a `_ =>` fallback cannot quietly misfile a new lift), and the
  prescription table is asserted per class.
- `src/exercise_progress.rs` — the double-progression cases: rep advance
  follows the worst set, load bump at the top of the range, hold on one
  miss, 10% deload on two, bodyweight rep-only, performed-weight basis,
  kg/lb snapping, overrides.
- `src/volume.rs` — the 7-day window, primary/secondary weighting, and the
  suggestion's deficit scoring and tie-break.
- `src/db/migration.rs` — old-schema fixtures for all three legacy
  programs, history-beats-state, template seeding, dropped tables, and
  double-run idempotency.

## Handler tests — the seams between RPCs

`src/server/workout_tests.rs` builds a real `ServerDb::new_in_dir` on a
temp directory and drives real RPCs through the whole loop: onboard →
home → start-from-template → complete → end → tracker advanced exactly
once → next home reflects it. The layoff deload (applied at StartWorkout,
never written), manual overrides and the kg unit path live here, because
they only appear **between** two handlers.

## API invariant harness

`examples/api_invariant_fuzz.rs` spawns its own backend on its own port with its
own SQLite file, then drives randomised but realistic sequences — onboard,
start from a template or from explicit groups, complete sets, skip sets,
delete completed sets, edit a group's plan mid-workout, reorder groups, end
the workout (sometimes twice) — with several users running concurrently.
...
| A re-fired `EndWorkout` moves no tracker | Double progression on retries |
