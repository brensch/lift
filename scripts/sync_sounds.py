...
def render_gen(files: list[Path]) -> str:
    """A revision that changes whenever any sound's bytes change. Android
    fixes a notification channel's sound at creation, so the channel id
    carries this and a changed sound gets a fresh channel."""
    h = hashlib.sha256()
    for f in files:
    ...
    return (
        ...
        f"const String soundsRevision = '{h.hexdigest()[:10]}';\n"
    )
...
def main() -> int:
    ...
    gen_new = render_gen(files)
    if not GEN.exists() or GEN.read_text() != gen_new:
        stale.append(("write", GEN))
