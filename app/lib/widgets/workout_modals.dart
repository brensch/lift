...
Future<void> showExerciseGroupDetailsSheet(
  BuildContext context, {
  required String title,
  required String explanation,
  required List<ExerciseTypeConfig> exerciseConfigs,
  required int fallbackSets,
}) {
  final unit = context.read<SettingsProvider>().weightUnit;
  final colorScheme = Theme.of(context).colorScheme;

  List<WorkingSetSpec> workingSetsForConfig(ExerciseTypeConfig cfg) {
    if (cfg.workingSets.isNotEmpty) return cfg.workingSets;
    final count = fallbackSets <= 0 ? 1 : fallbackSets;
  ...
  String formatSetLine(WorkingSetSpec set) {
  ...
  String setPlanSummary() {
    if (exerciseConfigs.isEmpty) return 'Set plan';
    final allWorking = exerciseConfigs
        .expand(workingSetsForConfig)
        .toList(growable: false);
    if (allWorking.isEmpty) return 'Set plan';

    final amrapCount = allWorking.where((s) => s.isAmrap).length;
    final warmupsIncluded = exerciseConfigs
        .where((c) => c.includeWarmup)
        .length;

    if (exerciseConfigs.length == 1) {
    ...
    return '${exerciseConfigs.length} exercises • ${allWorking.length} working sets • $repText reps$amrapText$warmupText';
  }
      ...
      child: DraggableScrollableSheet(
        ...
        builder: (context, controller) => ListView(
          ...
          children: [
            ...
            for (final cfg in exerciseConfigs)
              _ExerciseGroupDetailsCard(
                title: exerciseNames[cfg.exercise] ?? 'Exercise',
                setLines: [
                  for (int i = 0; i < workingSetsForConfig(cfg).length; i++)
                    (
                      index: i + 1,
                      text: formatSetLine(workingSetsForConfig(cfg)[i]),
                      note: workingSetsForConfig(cfg)[i].instruction,
                    ),
                ],
                meta: [
...
typedef _DetailsSetLine = ({int index, String text, String note});

class _ExerciseGroupDetailsCard extends StatelessWidget {
  final String title;
  final List<_DetailsSetLine> setLines;
  final List<String> meta;

  const _ExerciseGroupDetailsCard({
    required this.title,
