...
void main() {
  ...
  group('addPrescribedExercises', () {
    ...
    test(
      'replaying a queued add over adopted server state never duplicates',
      () async {
        await bootWorkout();
        h.service.failMutations = true; // hold the add in the queue
        await h.provider.addPrescribedExercises([
  ...
  group('adjustExerciseWeight', () {
    test(
      'moves pending working sets, leaves completed and warmups alone',
      () async {
        await bootWorkout();
        await h.provider.completeSet('s2', 6, 185);
        await h.provider.adjustExerciseWeight(Exercise.EXERCISE_SQUAT, 205);

        final byId = {for (final s in h.provider.activeProposedSets) s.id: s};
        expect(
          byId['s1']!.targetWeight,
          45,
          reason: 'warmup untouched locally',
        );
        expect(
          byId['s2']!.targetWeight,
          185,
          reason: 'completed set untouched',
        );
  ...
  group('removeExercise', () {
    test('cancels pending sets, keeps completed ones, blocks update', () async {
      ...
      expect(
        squatBlock.single.sets.map((s) => s.id),
        ['s2'],
  ...
  group('reorderExercises', () {
    test('moves whole blocks and renumbers orders', () async {
      ...
      final orders = h.provider.activeProposedSets
          .map((s) => s.workoutOrder)
          .toList();
      expect(
        orders,
  ...
  group('offline queue', () {
    ...
    test('the final set of an exercise gets the short local rest', () async {
      ...
      final lastDone = h.provider.activeCompletedSets.singleWhere(
        (c) => c.proposedSetId == 's3',
      );
