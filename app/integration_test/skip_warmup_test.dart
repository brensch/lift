...
void main() {
  ...
  testWidgets('workout — skipping a warmup cancels it in the backend', (
    ...
    expect(
      await s.waitForText('Start Set', seconds: 10),
      isTrue,
    ...
    final before =
        (await api.workoutDetail())?.proposedSets
            .where((x) => x.warmup)
            .length ??
        0;
    ...
    expect(
      await s.waitForText('Skip', seconds: 6),
      isTrue,
    ...
    expect(
      ...
      before - 1,
      reason:
          'skipping a warmup should remove exactly one warmup from the plan',
    );
    expect(
      s.isVisible('Start Set') || s.isVisible('End Workout'),
      isTrue,
