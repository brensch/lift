...
fn next_up_set_handles_mid_workout_weight_change_state() {
    ...
    let next = compute_next_up_set(&proposed_sets, &completed_sets).expect("next set should exist");
...
fn session_next_up_skips_active_lifter_and_prefers_ready_lifter() {
    ...
    let next = compute_session_next_up(&[p1, p2, p3], 100).expect("session next up should exist");
